...
// TypeScript module for Indian geographical data
...
export interface CityDropdownOptions {
    stateId: string;
    cityId: string;
}
...
// Processed cities data with proper typing
export const STATES_CITIES_DATA: readonly StateCity[] = INDIAN_STATES.map((state, index) => ({
    state,
    cities: CITIES_DATA[index]
        .split("|")
        .map(city => city.trim())
        .filter(city => city.length > 0)
}));

// Enhanced data structure compatible with React Select components
export const STATES_CITIES_FOR_REACT = INDIAN_STATES.map((state, index) => ({
    name: state,
    cities: CITIES_DATA[index]
        .split("|")
        .map(city => city.trim())
...
export class IndianCitiesUtil {

    /**
     * Get all cities for a specific state
     */
    static getCitiesByState(stateName: string): readonly string[] {
        const stateData = STATES_CITIES_DATA.find(item => item.state === stateName);
     ...
     */
    static getStateIndex(stateName: string): number {
        return INDIAN_STATES.findIndex(state => state === stateName);
    }

    /**
     * Search cities across all states
     */
    static searchCities(searchTerm: string): Array<{ state: string; city: string }> {
        const results: Array<{ state: string; city: string }> = [];
        const lowerSearchTerm = searchTerm.toLowerCase();

        STATES_CITIES_DATA.forEach(stateData => {
            stateData.cities.forEach(city => {
                if (city.toLowerCase().includes(lowerSearchTerm)) {
                    results.push({ state: stateData.state, city });
                }
            });
        });
    ...
    /**
     * Get all cities as a flat array with state information
     */
    static getAllCitiesWithStates(): Array<{ state: string; city: string }> {
        const allCities: Array<{ state: string; city: string }> = [];

        STATES_CITIES_DATA.forEach(stateData => {
            stateData.cities.forEach(city => {
                allCities.push({ state: stateData.state, city });
            });
        });
     ...
     */
    static isCityInState(cityName: string, stateName: string): boolean {
    ...
    /**
     * Get states that contain a specific city name
     */
    static getStatesByCity(cityName: string): readonly string[] {
        const states: string[] = [];
        const lowerCityName = cityName.toLowerCase();

        STATES_CITIES_DATA.forEach(stateData => {
            const hasCity = stateData.cities.some(city =>
                city.toLowerCase() === lowerCityName
            );
            if (hasCity) {
    ...
    /**
     * Convert to React Select compatible format
     */
    static getReactSelectData() {
        return STATES_CITIES_FOR_REACT;
    }

    /**
     * Get cities for a state in React Select format
     */
    static getCitiesForReactSelect(stateName: string): Array<{ value: string; label: string }> {
        const cities = this.getCitiesByState(stateName);
    ...
    /**
     * Get states in React Select format
     */
    static getStatesForReactSelect(): Array<{ value: string; label: string }> {
...
// HTML dropdown population functions (for vanilla JS applications)
export class DropdownPopulator {

    /**
     * Populate state dropdown
     */
    static populateStates(stateSelectId: string): void {
        const stateSelect = document.getElementById(stateSelectId) as HTMLSelectElement;
        if (!stateSelect) {
            console.error(`Element with ID '${stateSelectId}' not found`);
            return;
        }
     ...
     */
    static populateCities(citySelectId: string, selectedState: string): void {
        const citySelect = document.getElementById(citySelectId) as HTMLSelectElement;
        if (!citySelect) {
            console.error(`Element with ID '${citySelectId}' not found`);
            return;
        }
        ...
        // Add city options
        const cities = IndianCitiesUtil.getCitiesByState(selectedState);
        cities.forEach(city => {
            const option = document.createElement('option');
            option.value = city;
    ...
    /**
     * Setup automatic city population when state changes
     */
    static setupAutomaticPopulation(stateSelectId: string, citySelectId: string): void {
        const stateSelect = document.getElementById(stateSelectId) as HTMLSelectElement;
        if (!stateSelect) {
            console.error(`State select element with ID '${stateSelectId}' not found`);
            return;
        }

        stateSelect.addEventListener('change', (event) => {
            const target = event.target as HTMLSelectElement;
            this.populateCities(citySelectId, target.value);
        });
    }
}

// React hooks for use in React applications
export const useIndianStatesAndCities = () => {
    return {
        states: INDIAN_STATES,
        statesForReactSelect: IndianCitiesUtil.getStatesForReactSelect(),
        reactSelectData: IndianCitiesUtil.getReactSelectData(),
        getCitiesByState: IndianCitiesUtil.getCitiesByState,
        getCitiesForReactSelect: IndianCitiesUtil.getCitiesForReactSelect,
        searchCities: IndianCitiesUtil.searchCities,
        isCityInState: IndianCitiesUtil.isCityInState,
        getStatesByCity: IndianCitiesUtil.getStatesByCity,
        getAllCitiesWithStates: IndianCitiesUtil.getAllCitiesWithStates
    };
};

// Types for external use
export type StateName = typeof INDIAN_STATES[number];
export type CitySearchResult = { state: string; city: string };
