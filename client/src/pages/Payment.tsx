...
import { ArrowLeft, Check, Info, CreditCard, Lock, Shield } from "lucide-react";
...
const Payment = () => {
  ...
  const { orderQuote, isQuoteLoading, cartItems } = useCart();
  ...
  // Use authoritative totals from the context's orderQuote
  const orderTotal = orderQuote?.subtotal ?? 0;
  const shippingCost = orderQuote?.shippingCost ?? 0;
  const totalDiscount = orderQuote?.discounts?.total ?? 0;
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-semibold text-gray-800">
                        {formatRupees(orderTotal)}
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-gray-700">Shipping</span>
                      <span className="font-semibold text-gray-800">
                        {shippingCost > 0 ? formatRupees(shippingCost) : "Free"}
                      </span>
                    </div>
                    {totalDiscount > 0 && (
